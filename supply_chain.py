...
def generate_action_plan(analysis, performance_labels, recommendations):
    ...
    plan["week2"].append({"action": "Map your current supply chain process from start to finish and identify where time and resources are being lost", "priority": "Standard"})
    for kpi, result in critical_kpis:
...
def generate_pdf_report(company_name, category, industry, business_model, currency_symbol,
                          ...
                          root_causes, recommendations, priority_list, projections, action_plan_data=None):
    buffer = io.BytesIO()
